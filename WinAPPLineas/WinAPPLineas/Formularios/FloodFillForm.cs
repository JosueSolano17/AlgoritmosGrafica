...
namespace WinAPPLineas.Formularios
{
    public partial class FloodFillForm : Form
    {
        ...
        private Color fillColor = Color.FromArgb(102, 187, 106); // Verde suave
        private Color boundaryColor = Color.FromArgb(66, 165, 245); // Azul cielo
        private Color drawColor = Color.FromArgb(66, 165, 245);
        private Color explorationColor = Color.FromArgb(150, 255, 167, 38); // Naranja
        private Color backtrackColor = Color.FromArgb(150, 239, 83, 80); // Rojo coral
        ...
        private void InitializeCustomComponents()
        {
            ...
            this.Text = "Flood Fill - Rectángulo Animado";
            ...
            lblSpeedValue = new Label
            {
                ...
                Size = new Size(65, 20),
                Font = new Font("Segoe UI", 9F, FontStyle.Bold),
                ForeColor = Color.FromArgb(102, 187, 106),
            ...
            {
                Text = "Mostrar exploración de vecinos",
                Location = new Point(15, 25),
                Size = new Size(230, 25),
                Font = new Font("Segoe UI", 8.5F),
                ForeColor = Color.FromArgb(224, 224, 224),
            ...
            {
                Text = "Mostrar retrocesos (backtrack)",
                Location = new Point(15, 55),
                Size = new Size(230, 25),
                Font = new Font("Segoe UI", 8.5F),
                ForeColor = Color.FromArgb(224, 224, 224),
            ...
            grpVisualization.Controls.AddRange(new Control[] { chkShowExploration, chkShowBacktrack, chkShowNumbers });
            ...
            lblInstructions = new Label
            {
                Text = "📋 INSTRUCCIONES:\n\n" +
                       ...
                       "El algoritmo rellena desde\n" +
                       "el centro del rectángulo.",
                Location = new Point(10, 840),
                Size = new Size(260, 140),
                Font = new Font("Segoe UI", 8.5F),
                ForeColor = Color.FromArgb(224, 224, 224),
                BackColor = Color.FromArgb(48, 48, 48),
        ...
        private void BtnDrawRectangle_Click(object sender, EventArgs e)
        {
            ...
            // Dibujar el rectángulo en la cuadrícula
            ...
            for (int y = 0; y < gridHeight; y++)
            {
                for (int x = 0; x < gridWidth; x++)
                {
                    ...
                    if (grid[y, x] == 1) // Límite
                    {
                        ...
                        {
                            DrawCellNumber(x, y, "1", Color.White);
                        }
                    }
        ...
        private async void BtnStartFill_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                await AnimateFillAsync(center.X, center.Y, cancellationTokenSource.Token);
            }
        ...
        private async Task AnimateFillAsync(int sx, int sy, CancellationToken token)
        {
            ...
            while (stack.Count > 0)
            {
                ...
                foreach (var d in directions)
                {
                    ...
                    if (queued.Contains(np)) continue;

                    if (chkShowExploration.Checked)
                    {
                        DrawArrow(x, y, nx, ny, Color.Green);
                        await Task.Delay(delay / 3, token);
                    }
                ...
                {
                    if (!queued.Contains(np) && grid[np.Y, np.X] == 0)
                    {
                        queued.Add(np);
                        parent[np] = p;
                        stack.Push(np);
                ...
                // Mostrar backtrack si no hay nuevos vecinos
                if (newNeighbors.Count == 0 && chkShowBacktrack.Checked)
                {
                    if (parent.ContainsKey(p) && parent[p].X >= 0)
                    {
                        var par = parent[p];
                        DrawArrow(x, y, par.X, par.Y, Color.Yellow);
                        await Task.Delay(delay / 2, token);
                    }
                }
        ...
        private void DrawCell(int gridX, int gridY, Color color, bool showNumber)
        {
            ...
            if (showNumber)
            {
                DrawCellNumber(gridX, gridY, "1", Color.White);
            }
        ...
        private void DrawCellNumber(int gridX, int gridY, string text, Color color)
        {
            ...
            using (Font font = new Font("Consolas", Math.Max(6f, cellSize * 0.6f), FontStyle.Bold))
        ...
        private void FloodFillForm_Load(object sender, EventArgs e)
        {
