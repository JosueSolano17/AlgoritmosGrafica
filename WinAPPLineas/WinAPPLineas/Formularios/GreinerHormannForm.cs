...
{
    public partial class GreinerHormannForm : Form
    {
        ...
        private Color subjectColor = Color.FromArgb(33, 150, 243); // Azul
        private Color clipColor = Color.FromArgb(255, 152, 0); // Naranja
        private Color resultColor = Color.FromArgb(76, 175, 80); // Verde
        ...
        private void InitializeCustomComponents()
        {
            ...
            {
                Text = "Polígono de Recorte (Naranja)",
        ...
        private void DrawPolygon(List<PointD> polygon, Color color, string label, bool showNumbers, bool filled = false)
        {
            ...
            // Dibujar bordes
            using (Pen pen = new Pen(color, filled ? 3 : 2))
            {
                if (polygon.Count >= 3)
                {
            ...
            for (int i = 0; i < polygon.Count; i++)
            {
                ...
                if (showNumbers)
                {
                    ...
                    using (SolidBrush bgBrush = new SolidBrush(Color.FromArgb(180, color)))
                    {
        ...
        private void BtnAddSubjectVertex_Click(object sender, EventArgs e)
        {
            ...
            btnAddSubjectVertex.BackColor = isAddingSubject ? Color.FromArgb(0, 200, 83) : Color.FromArgb(33, 150, 243);
            btnAddSubjectVertex.Text = isAddingSubject ? "✓ Agregando... (Click para Detener)" : "➕ Click en Canvas para Agregar";
            btnAddClipVertex.BackColor = Color.FromArgb(255, 152, 0);
        ...
        private void BtnAddClipVertex_Click(object sender, EventArgs e)
        {
            ...
            btnAddClipVertex.BackColor = isAddingClip ? Color.FromArgb(0, 200, 83) : Color.FromArgb(255, 152, 0);
            btnAddClipVertex.Text = isAddingClip ? "✓ Agregando... (Click para Detener)" : "➕ Click en Canvas para Agregar";
            btnAddSubjectVertex.BackColor = Color.FromArgb(33, 150, 243);
        ...
        private void BtnLoadPreset_Click(object sender, EventArgs e)
        {
            ...
            // Polígono sujeto: Cuadrado
            subjectPolygon.Add(new PointD(-100, -100));
            subjectPolygon.Add(new PointD(100, -100));
            subjectPolygon.Add(new PointD(100, 100));
            subjectPolygon.Add(new PointD(-100, 100));

            // Polígono de recorte: Rectángulo rotado
            clipPolygon.Add(new PointD(-50, -150));
        ...
        private async void BtnClip_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                {
                    await AnimateGreinerHormann(cancellationTokenSource.Token);
                }
                else
                {
                    PerformClipping();
        ...
        private async Task AnimateGreinerHormann(CancellationToken token)
        {
            int delay = GetDelayFromTrackbar();

            var steps = CRecortarPoligono.TraceGreinerHormann(subjectPolygon, clipPolygon);
            ...
            foreach (var step in steps)
            {
                ...
                switch (step.Action)
                {
                    case GreinerHormannStep.StepAction.Start:
                        message = step.Message;
                        break;

                    case GreinerHormannStep.StepAction.FindIntersections:
                        message = step.Message;

                        // Extraer coordenadas de intersección del mensaje
                        if (chkShowIntersections.Checked && step.Message.Contains("("))
                        {
                            RedrawDisplay();
                            // Dibujar marca temporal de intersección
                        }
                        break;

                    case GreinerHormannStep.StepAction.BuildGraph:
                        message = step.Message;
                        break;

                    case GreinerHormannStep.StepAction.FinalResult:
                        message = step.Message;
        ...
        private void PerformClipping()
        {
            ...
            foreach (var step in steps)
            {
                ...
                if (step.Action == GreinerHormannStep.StepAction.FinalResult && step.ResultSoFar != null)
                {
                    resultPolygon = new List<PointD>(step.ResultSoFar);
        ...
        private void GreinerHormannForm_Load(object sender, EventArgs e)
        {
