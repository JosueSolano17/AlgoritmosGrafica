...
{
    public partial class SutherlandHodgmanForm : Form
    {
        ...
        private Color subjectColor = Color.FromArgb(255, 87, 34); // Naranja profundo
        private Color clipColor = Color.FromArgb(156, 39, 176); // Púrpura
        private Color resultColor = Color.FromArgb(0, 150, 136); // Teal
        private Color currentEdgeColor = Color.FromArgb(255, 193, 7); // Amarillo
        private Color gridColor = Color.FromArgb(200, 200, 200);
        private Color bgColor = Color.FromArgb(250, 250, 250);
        private Color vertexColor = Color.FromArgb(233, 30, 99); // Rosa
        ...
        private PointD currentClipEdgeA;
        private PointD currentClipEdgeB;

        public SutherlandHodgmanForm()
        {
            InitializeComponent();
        ...
        private void InitializeCustomComponents()
        {
            ...
            {
                Text = "Polígono de Recorte (Púrpura)",
            ...
            chkShowEdges = new CheckBox
            {
                Text = "Mostrar arista de recorte actual",
                Location = new Point(10, 25),
                Size = new Size(310, 20),
                Font = new Font("Segoe UI", 8.5F),
                ForeColor = Color.FromArgb(224, 224, 224),
            ...
            grpVisualization.Controls.AddRange(new Control[] { chkShowEdges, chkShowVertexNumbers, chkShowGrid });
            ...
            lblInstructions = new Label
            {
                Text = "📋 SUTHERLAND-HODGMAN:\n\n" +
                       "Algoritmo clásico para\n" +
                       "recortar polígonos convexos.\n\n" +
                       "PASOS:\n" +
                       "1. Agrega vértices al\n" +
                       "   polígono sujeto (naranja)\n" +
                       "2. Agrega vértices al\n" +
                       "   polígono de recorte\n" +
                       "   (púrpura) - debe ser CONVEXO\n" +
                       "3. Presiona 'Recortar'\n\n" +
                       "Procesa cada arista del\n" +
                       "polígono de recorte\n" +
                       "secuencialmente.",
        ...
        private void RedrawDisplay()
        {
            ...
            // Dibujar arista actual de recorte si está en animación
            if (chkShowEdges.Checked && currentClipEdgeA.X != 0 && currentClipEdgeB.X != 0)
            {
                DrawCurrentEdge(currentClipEdgeA, currentClipEdgeB);
            }
        ...
        private void DrawCurrentEdge(PointD a, PointD b)
        {
            var pa = WorldToScreen(a);
            var pb = WorldToScreen(b);

            using (Pen pen = new Pen(currentEdgeColor, 4))
            {
                pen.DashStyle = DashStyle.Dash;
                displayGfx.DrawLine(pen, pa, pb);
            }

            // Dibujar flechas en la dirección del edge
            DrawArrowHead(pa, pb, currentEdgeColor);
        }

        private void DrawArrowHead(PointF from, PointF to, Color color)
        {
            float angle = (float)Math.Atan2(to.Y - from.Y, to.X - from.X);
            float arrowLength = 15;
            float arrowAngle = 25 * (float)Math.PI / 180;
            ...
            using (SolidBrush brush = new SolidBrush(color))
            {
                displayGfx.FillPolygon(brush, new PointF[] { to, p1, p2 });
            }
        }
        ...
        private void BtnAddSubjectVertex_Click(object sender, EventArgs e)
        {
            ...
            btnAddSubjectVertex.BackColor = isAddingSubject ? Color.FromArgb(0, 200, 83) : Color.FromArgb(255, 87, 34);
            btnAddSubjectVertex.Text = isAddingSubject ? "✓ Agregando... (Click para Detener)" : "➕ Click en Canvas para Agregar";
            btnAddClipVertex.BackColor = Color.FromArgb(156, 39, 176);
        ...
        private void BtnAddClipVertex_Click(object sender, EventArgs e)
        {
            ...
            btnAddClipVertex.BackColor = isAddingClip ? Color.FromArgb(0, 200, 83) : Color.FromArgb(156, 39, 176);
            btnAddClipVertex.Text = isAddingClip ? "✓ Agregando... (Click para Detener)" : "➕ Click en Canvas para Agregar";
            btnAddSubjectVertex.BackColor = Color.FromArgb(255, 87, 34);
        ...
        private void BtnLoadPreset_Click(object sender, EventArgs e)
        {
            ...
            // Polígono sujeto: Pentágono
            subjectPolygon.Add(new PointD(0, 120));
            subjectPolygon.Add(new PointD(114, 37));
            subjectPolygon.Add(new PointD(71, -97));
            subjectPolygon.Add(new PointD(-71, -97));
            subjectPolygon.Add(new PointD(-114, 37));

            // Polígono de recorte: Rectángulo (CONVEXO)
            clipPolygon.Add(new PointD(-150, -50));
            clipPolygon.Add(new PointD(150, -50));
            clipPolygon.Add(new PointD(150, 50));
            clipPolygon.Add(new PointD(-150, 50));
        ...
        private async void BtnClip_Click(object sender, EventArgs e)
        {
            ...
            resultPolygon.Clear();
            currentClipEdgeA = new PointD(0, 0);
            currentClipEdgeB = new PointD(0, 0);

            try
            {
                if (chkAnimate.Checked)
                {
                    await AnimateSutherlandHodgman(cancellationTokenSource.Token);
                }
                else
                {
                    PerformClipping();
        ...
        private async Task AnimateSutherlandHodgman(CancellationToken token)
        {
            int delay = GetDelayFromTrackbar();

            var steps = CRecortarPoligono.TraceSutherlandHodgman(subjectPolygon, clipPolygon);
            ...
            foreach (var step in steps)
            {
                ...
                switch (step.Action)
                {
                    case SutherlandHodgmanStep.StepAction.Start:
                        message = step.Message;
                        break;

                    case SutherlandHodgmanStep.StepAction.EdgeStart:
                        message = step.Message;
                        currentClipEdgeA = step.ClipA;
                        currentClipEdgeB = step.ClipB;
                        if (step.ResultSoFar != null)
                        {
                    ...
                    case SutherlandHodgmanStep.StepAction.TestPoint:
                        message = step.Message;
                        break;

                    case SutherlandHodgmanStep.StepAction.IntersectionComputed:
                        message = step.Message;
                        if (step.ResultSoFar != null)
                        {
                            resultPolygon = new List<PointD>(step.ResultSoFar);
                    ...
                    case SutherlandHodgmanStep.StepAction.OutputVertex:
                        message = step.Message;
                        if (step.ResultSoFar != null)
                        {
                            resultPolygon = new List<PointD>(step.ResultSoFar);
                    ...
                    case SutherlandHodgmanStep.StepAction.EdgeFinished:
                        message = step.Message;
                        if (step.ResultSoFar != null)
                        {
                            resultPolygon = new List<PointD>(step.ResultSoFar);
                    ...
                    case SutherlandHodgmanStep.StepAction.FinalResult:
                        message = step.Message;
                        if (step.ResultSoFar != null)
                        {
                            resultPolygon = new List<PointD>(step.ResultSoFar);
        ...
        private void PerformClipping()
        {
            ...
            resultPolygon = CRecortarPoligono.SutherlandHodgmanClip(subjectPolygon, clipPolygon);

            lstSteps.Items.Add($"Recorte completado: {resultPolygon.Count} vértices");
        ...
        private void SutherlandHodgmanForm_Load(object sender, EventArgs e)
        {
